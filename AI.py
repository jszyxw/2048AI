...
def move(grid, rot):
    ...
    for i in range(rot):
        tmp = rotateRight(tmp)
    out = [move_row(row) for row in tmp]
    return out, out != tmp
...
def getNextMove(grid):
    print(len([search_min(move(grid,m)[0]) for m in range4 if move(grid,m)[1]]))
    return ['a','w','d','s'][max((search_min(move(grid,m)[0]),m) for m in range4 if move(grid,m)[1])[1]]
